// Heatmap-style visualization: fills a large rectangle with tiles where color
// encodes the tariff rate (light red = low, dark red = high). This replaces
// the prior treemap behavior and keeps the module name so renderer wiring stays intact.
(function(){
    window.VizTreemap = {};
    ...
    VizTreemap.draw = function(p, manager, ai, progress){
        ...
        // Area-proportional partition layout (binary split). This fills the
        // container and gives each item an area proportional to its value.
        function sumValues(arr){ return arr.reduce(function(s,it){ return s + it.value; }, 0); }

        // Recursively partition items into rectangles. Splits items into two groups
        // whose sums are as balanced as possible, then divides the area accordingly.
        var rects = [];
        function partition(itemsList, x, y, w, h, vertical){
            ...
            // split into leftGroup = 0..splitIndex, rightGroup = splitIndex+1..
